...
public class Disks {
    ...
    public boolean checkHit(Player.Name name) {
        for (int col = 0 ; col < 8 ; col++){
            for (int row = 0 ; row < 8 ; row++){
    ...
    public int checkRight(Player.Name color, Coordinate coordinate) {
        ...
        for (col++ ; col < 8 ; col++){
            if (listOfDiskList.get(col).get(row).getFill() == Color.TRANSPARENT) return 0;
            if (listOfDiskList.get(col).get(row).getFill() == color.getColor()) break;
        }
